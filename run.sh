...
case "$1" in
  start)
    ...
    pm2 start ${APP_FOLDER}/server.js --name grabatix --uid bitnami --gid bitnami
    exit $?
    ;;
  stop)
    pm2 stop grabatix --uid bitnami --gid bitnami
    exit $?
    ;;
  restart|force-reload|reload)
    pm2 restart grabatix --uid bitnami --gid bitnami
    exit $?
    ;;
  init)
